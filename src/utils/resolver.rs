...
pub async fn load_wz_by_base(
    ...
    append_to_log(&format!("Base.wz version: {} \n", patch_version)).await?;
...
pub async fn resolve_pack_ms(path: &str, base_node: &WzNodeArc) -> Result<()> {
    ...
    if first_parent.file_stem().unwrap() != "Base" {
        append_to_log(&"older structure, skip Pack folder\n").await?;
        return Ok(());
    }
    ...
    if pack_entry.is_err() {
        append_to_log(&"Pack folder not found\n").await?;
        return Ok(());
    }
